# プロジェクト: 新崎川 水位＋雨量ロガー
（Claude Code・Codex 共通の指示書です。ルールはこのファイルにだけ書く）
...
## このプロジェクトの例外
- このリポジトリは公開情報（県の雨量・水位データ）だけを扱うため、例外として**Public**にする。
  「GitHubリポジトリは必ずPrivate」という全体ルールはこのリポジトリには当てはめない。
- Webアプリではない（Pythonスクリプト＋GitHub Actions）ため、「Next.js＋Cloudflare以外の構成は提案しない」という構成の決まりも当てはまらない。
